  ...
  <section class="py-8 md:py-10 auth-page">
    <div class="container-shell space-y-6 page-entry">
        ...
        <div class="min-w-0 flex-1">
          ...
          <div v-else-if="!products.length" class="surface-card flex flex-col items-center gap-3 p-12 text-center">
            <div class="flex h-16 w-16 items-center justify-center rounded-2xl bg-slate-800 ring-1 ring-slate-700">
              <svg class="h-8 w-8 text-slate-600" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.4">
          ...
          <div v-else class="space-y-4">
            ...
            <!-- Featured card (single product or first featured) -->
                ...
                <div class="flex flex-col md:flex-row">
                  ...
                  <div class="relative w-full overflow-hidden md:w-64 md:shrink-0">
                    ...
                    <div v-if="discountPct(product)" class="absolute left-3 top-3">
                      <span class="flex items-center gap-1 rounded-full bg-amber-400 px-2.5 py-1 text-xs font-black text-amber-950 shadow-lg shadow-amber-400/20">
                        {{ t('shop.off', { n: discountPct(product) }) }}
                      </span>
                    </div>

                    <!-- In cart badge -->
                    <div v-if="cart[product.id]" class="absolute right-3 top-3">
                      <span class="flex h-7 w-7 items-center justify-center rounded-full bg-violet-600 text-xs font-black text-white shadow-lg">{{ cart[product.id] }}</span>
                    </div>
                  </div>
                  ...
                  <div class="flex flex-1 flex-col justify-between gap-4 p-5 md:p-6">
                    <div class="space-y-3">
                      ...
                      <p v-if="product.description" class="text-sm leading-relaxed text-slate-400">{{ product.description }}</p>
                    </div>
                    ...
                    <div class="flex flex-wrap items-end justify-between gap-4">
                      ...
                      <div class="space-y-0.5">
                        ...
                        <div class="text-3xl font-black tracking-tight text-slate-50">
                          {{ Number(product.price).toLocaleString('ru') }} <span class="text-xl text-slate-400">₽</span>
                        </div>
                      </div>
                      ...
                      <div class="flex items-center gap-2">
                        <button
                          ...
                          class="flex h-10 w-10 items-center justify-center rounded-xl bg-slate-700 text-lg font-bold text-slate-200 transition hover:bg-slate-600 active:scale-95"
                        ...
                        <button
                          ...
                          :class="cart[product.id] ? 'bg-violet-700 hover:bg-violet-600' : 'bg-violet-600 hover:bg-violet-500 shadow-lg shadow-violet-600/20'"
                        ...
                        >
                          ...
                          {{ cart[product.id] ? t('shop.buyBtn') : t('shop.buyBtn') }}
                        </button>
                      </div>
                    </div>
                  </div>
          ...
          <div class="sticky top-20 overflow-hidden rounded-[20px] bg-slate-800/60 ring-1 ring-slate-700/50">
            ...
            <div class="p-4 space-y-4">
              ...
              <input
                v-if="cartCount"
                v-model="coupon"
                class="w-full rounded-xl border border-slate-700 bg-slate-900 px-3 py-2 text-xs text-slate-200 outline-none placeholder:text-slate-600 focus:border-violet-500 transition"
              ...
              <button
                ...
                class="w-full rounded-xl bg-violet-600 py-2.5 text-sm font-bold text-white transition hover:bg-violet-500 shadow-lg shadow-violet-600/20 active:scale-[.98]"
                @click="router.push({ path: '/login', query: { redirect: '/shop' } })"
              >{{ t('shop.loginToBuy') }}</button>

              <button
                v-else
                class="w-full rounded-xl py-2.5 text-sm font-bold text-white transition active:scale-[.98]"
                :class="cartCount && !paying
                  ? 'bg-gradient-to-r from-violet-600 to-indigo-600 hover:from-violet-500 hover:to-indigo-500 shadow-lg shadow-violet-600/20'
              ...
              <button v-if="cartCount" class="w-full text-xs text-slate-600 transition hover:text-slate-400" @click="clearCart">
                {{ t('shop.clearCart') }}
              </button>
              ...
              <div v-if="auth.isAuthenticated.value" class="rounded-xl border border-slate-700/40 bg-slate-900/60 p-3 text-xs space-y-1.5">
                ...
                <div class="flex items-center gap-2">
                  <div class="flex h-6 w-6 shrink-0 items-center justify-center rounded-lg bg-violet-600/20 text-xs font-black text-violet-300">
                    {{ (auth.state.playerAccount?.minecraft_nickname || '?')[0].toUpperCase() }}
                  </div>
              ...
              <p class="border-t border-slate-700/40 pt-3 text-xs leading-relaxed text-slate-600">
                ...
                Цифровые товары не подлежат возврату.
              </p>
            </div>
          </div>
        </div>
        ...
        <div class="surface-card overflow-hidden p-0">
          <div class="divide-y divide-slate-700/30">
            <div
              ...
              :key="pay.id"
              class="flex items-center gap-3 px-4 py-3 transition hover:bg-slate-800/40"
            >
              <!-- Avatar -->
              <div class="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-gradient-to-br from-violet-600/30 to-indigo-600/20 ring-1 ring-slate-700 text-xs font-black text-violet-300">
      ...
      <div class="surface-card flex items-start gap-3 p-4 text-xs leading-relaxed text-slate-500">
        ...
        <p>
          Все товары — виртуальные игровые объекты без реальной стоимости. Покупка не даёт преимуществ над другими игроками в плане геймплея.
