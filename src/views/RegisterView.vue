...
async function submit() {
  ...
  try {
    ...
    errorMessage.value = error.message || 'Не удалось зарегистрировать аккаунт.'
  } finally {
  ...
  <section class="py-16 md:py-24">
    <div class="container-shell max-w-3xl">
      <div class="glass-card rounded-[32px] p-8 md:p-10">
        ...
        <h1 class="section-title">Создай единый аккаунт VoidRP</h1>
        <p class="section-subtitle">
          Этот аккаунт станет основой для сайта, лаунчера и будущего игрового входа без ручного /login.
        </p>

        <form class="mt-8 grid gap-4 md:grid-cols-2" @submit.prevent="submit">
          <label class="form-control w-full">
            <span class="label-text mb-2 font-semibold text-slate-700">Логин сайта</span>
            <input v-model="form.site_login" class="input input-bordered w-full rounded-2xl" required />
          </label>

          <label class="form-control w-full">
            <span class="label-text mb-2 font-semibold text-slate-700">Minecraft-ник</span>
          ...
          <label class="form-control w-full">
            <span class="label-text mb-2 font-semibold text-slate-700">Повтор пароля</span>
